...
$this->title = 'AO Profiles';

// Search/filter values used by the filter form.
// They are also safely read from GET if the controller does not pass them explicitly.
$search = $search ?? Yii::$app->request->get('search', '');
$status = $status ?? Yii::$app->request->get('status', '');
$emailVerified = $emailVerified ?? Yii::$app->request->get('email_verified', '');
...
$this->registerCss("
    ...
    /* Complete information display: no ellipsis, no hidden text */
    .info-cell {
        min-width: 120px;
    ...
    /* Actions column: display 3 buttons per row */
    .action-cell {
 ...
 * Yii2 keeps data-method='post', so actions are sent as POST.
 */
$this->registerJs(<<<JS
        ...
        <div class="page-header-card">
            <div>
                ...
                <div class="subtitle-text">
                    Manage AO profiles, company details, verification, and account status
                </div>
            </div>

            <?= Html::a(
                '<i class="bi bi-plus-circle"></i> Add AO Profile',
                ['create'],
        ...
        <div class="content-card">
            <div class="table-responsive-custom">
                <table class="table requests-table">
                    ...
                    <tbody>
                        <?php if (!empty($aoProfiles)): ?>
                            <?php foreach ($aoProfiles as $aoProfile): ?>
                                <?php
                                    $currentStatus = strtolower((string) $aoProfile->status);
                                    $statusClass = 'status-badge';

                                    if (in_array($currentStatus, ['active', 'approved', 'enabled'], true)) {
                                        $statusClass .= ' status-active';
                                    } elseif ($currentStatus === 'pending') {
                                        $statusClass .= ' status-pending';
                                    } elseif (in_array($currentStatus, ['banned', 'rejected', 'disabled'], true)) {
                                ...
                                <tr>
                                    <td data-label="ID">
                                        <?= Html::a(
                                            '#' . Html::encode($aoProfile->ao_id),
                                            ['view', 'id' => $aoProfile->ao_id],
                                            ['class' => 'request-link']
                                        ) ?>
                                    </td>
                                    ...
                                    <td data-label="Profile Photo">
                                        <?php if (!empty($aoProfile->profile_photo)): ?>
                                            <?= Html::img('@web/' . $aoProfile->profile_photo, [
                                                'class' => 'profile-photo',
                                                'alt' => 'Profile Photo',
                                            ]) ?>
                                        <?php else: ?>
                                    ...
                                    <td data-label="Username" class="info-cell">
                                        <?php if (!empty($aoProfile->username)): ?>
                                            <span class="user-badge text-badge-full">
                                                <i class="bi bi-person-circle"></i>
                                                <span class="text-content">
                                                    <?= Html::encode($aoProfile->username) ?>
                                                </span>
                                            </span>
                                        <?php else: ?>
                                    ...
                                    <td data-label="First Name" class="info-cell">
                                        <?php if (!empty($aoProfile->first_name)): ?>
                                            <span class="name-badge text-badge-full">
                                                <span class="text-content">
                                                    <?= Html::encode($aoProfile->first_name) ?>
                                                </span>
                                            </span>
                                        <?php else: ?>
                                            <span class="empty-badge">N/A</span>
                                    ...
                                    <td data-label="Last Name" class="info-cell">
                                        <?php if (!empty($aoProfile->last_name)): ?>
                                            <span class="name-badge text-badge-full">
                                                <span class="text-content">
                                                    <?= Html::encode($aoProfile->last_name) ?>
                                                </span>
                                            </span>
                                        <?php else: ?>
                                            <span class="empty-badge">N/A</span>
                                    ...
                                    <td data-label="Contact Number" class="info-cell">
                                        <?php if (!empty($aoProfile->contact_number)): ?>
                                            <span class="phone-badge text-badge-full">
                                                <i class="bi bi-telephone"></i>
                                                <span class="text-content">
                                                    <?= Html::encode($aoProfile->contact_number) ?>
                                                </span>
                                            </span>
                                        <?php else: ?>
                                            <span class="empty-badge">N/A</span>
                                    ...
                                    <td data-label="Email" class="info-cell info-cell-wide">
                                        <?php if (!empty($aoProfile->email)): ?>
                                            <span class="email-badge text-badge-full">
                                                <i class="bi bi-envelope"></i>
                                                <span class="text-content">
                                                    <?= Html::encode($aoProfile->email) ?>
                                                </span>
                                            </span>
                                        <?php else: ?>
                                            <span class="empty-badge">N/A</span>
                                    ...
                                    <td data-label="Company Name" class="info-cell info-cell-wide">
                                        <?php if (!empty($aoProfile->company_name)): ?>
                                            <span class="company-badge text-badge-full">
                                                <i class="bi bi-building"></i>
                                                <span class="text-content">
                                                    <?= Html::encode($aoProfile->company_name) ?>
                                                </span>
                                            </span>
                                        <?php else: ?>
                                            <span class="empty-badge">N/A</span>
                                    ...
                                    <td data-label="Status">
                                        <span class="<?= Html::encode($statusClass) ?>">
                                            ...
                                            <?= Html::encode(ucfirst((string) $aoProfile->status)) ?>
                                        </span>
                                    </td>

                                    <td data-label="Email Verified">
                                        <?php if ($aoProfile->email_verified): ?>
                                    ...
                                    <td data-label="Actions" class="action-cell">
                                        <div class="action-buttons">
                                            ...
                                            <?= Html::a(
                                                ...
                                                ['delete', 'id' => $aoProfile->ao_id],
                                                [
                                                    'class' => 'action-btn btn-delete',
                                                    'title' => 'Delete',
                                                    'data-bs-toggle' => 'tooltip',
                                                    'data' => [
                                                        'confirm' => 'AO profile #' . $aoProfile->ao_id . ' will be permanently deleted.',
                                                        'method' => 'post',
                                                    ],
                                                ]
                                            ) ?>
                                            ...
                                            <?= Html::a(
                                                ...
                                                ['toggle-status', 'id' => $aoProfile->ao_id],
                                                [
                                            ...
                                            <?= Html::a(
                                                '<i class="bi bi-key"></i>',
                                                ['send-password-reset', 'id' => $aoProfile->ao_id],
                                                [
