...
$this->registerCss("
    ...
    .city-create-page .container-fluid {
        max-width: 100%;
        padding-left: 0;
        padding-right: 0;
    }
...
$(document).ready(function () {
    ...
    function validateCityForm() {
        var isValid = true;
        var cityName = $.trim($('#city-name-input').val());
        var countryId = $('#country-select').val();

        clearFieldError('#city-name-input', '#city-name-error');
        clearFieldError('#country-select', '#country-error');
        $('.select2-container--default .select2-selection--single').removeClass('input-error');

        if (!cityName) {
            showFieldError('#city-name-input', '#city-name-error', 'Please enter the city name.');
            isValid = false;
        }

        if (!countryId) {
            $('#country-select').next('.select2-container').find('.select2-selection--single').addClass('input-error');
            $('#country-error').html('<i class="bi bi-exclamation-circle"></i><span>Please select a country.</span>').css('display', 'flex');
            isValid = false;
        }
    ...
    $('#country-select').on('change', function () {
        if ($(this).val()) {
            $('#country-select').next('.select2-container').find('.select2-selection--single').removeClass('input-error');
            $('#country-error').hide().html('');
        }
    });

    // Confirm form submission with SweetAlert2 after validation.
    $('#city-create-form').on('beforeSubmit', function () {
        var form = $(this);
        ...
        Swal.fire({
            ...
            html: 'Are you sure you want to add this city?',
            icon: 'question',
...
<main class="dash-content city-create-page can-form-page">
        ...
        <div class="page-header-card">
            <div>
                ...
                <div class="subtitle-text">
                    Create a new city and assign it to the correct country.
                </div>
            </div>

            <?= Html::a(
                '<i class="bi bi-arrow-left"></i> Back to City',
                ['index'],
                ['class' => 'btn btn-back-city']
            ) ?>
        </div>
                ...
                <?php $form = ActiveForm::begin([
                    ...
                    <div class="form-row-center">
                        <div class="form-column">
                            ...
                            <!-- City name field -->
                            <?= $form->field($city, 'city_name')->textInput([
                                'maxlength' => true,
                                'class' => 'form-control',
                                'id' => 'city-name-input',
                                'placeholder' => 'Enter city name',
                            ])->label(
                                $city->getAttributeLabel('city_name') . '<span class="required-star">*</span>',
                                ['encode' => false]
                            ) ?>

                            <div class="field-message">
                                <i class="bi bi-info-circle"></i>
                                <span>Enter the official city name as it should appear in airport and profile records.</span>
                            </div>

                            <div id="city-name-error" class="field-error-message"></div>
                            ...
                            <?= $form->field($city, 'country_id')->dropDownList(
                                ...
                                'Country Name<span class="required-star">*</span>',
                                ['encode' => false]
                            ) ?>

                            <div class="field-message">
                                <i class="bi bi-info-circle"></i>
                                <span>Select the country where this city is located.</span>
                            </div>
                    ...
                    <div class="form-actions">
                        <?= Html::submitButton(
                            '<i class="bi bi-check-circle"></i> Add City',
                            ['class' => 'btn btn-submit-city']
                        ) ?>

                        <?= Html::a(
                            '<i class="bi bi-arrow-left-circle"></i> Back to City',
                            ['index'],
                            ['class' => 'btn btn-back-city']
                        ) ?>
                    </div>
